...
def client():
    ...
    response = test_app.post(
        "/flowsheets/set_project",
        json={"project": "nawi", "data_location": "user_home"},
    )
    print(response)
    return test_app
    ...
    if "BSM2_P" not in flowsheet_id:
        ...
        for var_name, var_data in update_body["exports"].items():
            ...
            if (
                ...
                and var_data["is_input"]
                and not var_data["is_readonly"]
            ):
...
def test_save_config(client, flowsheet_id):
    ...
    response, body = post_flowsheet(
        ...
        "save",
        config,
        query_params={"name": "test name!", "version": "1"},
    )
    assert response.status_code == 200, body
    assert body == "test name!"

    response, body = post_flowsheet(
        client, flowsheet_id, "save", config, query_params={"version": "1"}
    )
    assert response.status_code == 200, body
    assert body == "current"
...
def test_list_configs(client, flowsheet_id):
    ...
    response, body = get_flowsheet(
        client, flowsheet_id, "list", query_params={"version": "1"}
    )
    assert response.status_code == 200
    current_names = body
    # add some more names
    ...
    for name in add_names:
        response, body = post_flowsheet(
            ...
            "save",
            config,
            query_params={"name": name, "version": "1"},
        )
